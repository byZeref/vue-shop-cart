<template>
    ...
    <div class="modal max-md:modal-bottom">
        <div v-if="!cartStore.empty" class="modal-box w-full md:max-w-3xl">
            ...
            <!-- ERROR ALERT -->
            <div v-if="cartStore.cart.some(el => el.buyValidated === false)"
            ...
            <div class="w-full">
                <div v-for="(item) in cartStore.cart" :key="item.id"
                    ...
                    <div class="basis-1/3 md:basis-1/4 px-1 md:px-2 py-4">
                        <p :class="[!item.buyValidated ? 'text-error' : 'text-slate-700',
                        'text-base md:text-lg font-bold']">
                            {{ item.product }}
                        </p>
                        <p :class="[!item.buyValidated ? 'text-error' : 'text-slate-500',
                        'max-md:text-sm']">
                            Cantidad: {{ item.cant }}
                        </p>
                    </div>
        ...
        <div v-else class="modal-box w-full md:max-w-3xl">
            <div :class="`text-${modalInfoCss}`">
                <p class="flex items-center font-semibold text-sm md:text-lg">
                    <InformationCircleIcon v-if="buyMsgeCode !== 1 && buyMsgeCode !== 2" class="w-7 h-7 mr-1" />
...
import { PlusIcon, MinusIcon, CheckCircleIcon, XCircleIcon, InformationCircleIcon } from '@heroicons/vue/24/outline'
